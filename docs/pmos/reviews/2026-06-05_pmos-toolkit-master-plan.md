...
| T3.2 | **W9b** comments — resolve identity (recommend: first-class skill), add gates | `[me/skill]` | ✅ DONE — first-class (`user-invocable: true`), 15/15 [D], prose thinned to defer to scripts. **Flagged → ✅ RESOLVED (2026-06-05):** resolver.js migrated sidecar→inline — reads/writes threads in the artifact's `<script id="pmos-comments">` block (mirrors serve.js: `COMMENTS_RE` + `jsonInlineEscape` + temp-then-rename atomic write, `version` bumped, `git add <artifact>` only; sidecar I/O + `_sidecarPathFor`/`_serializeSidecar` removed). Migration **exposed a latent anchor bug** (`_mainDocBounds` never matched the real `pmos-artifact-body` class → full-html search → a thread's own stored `quote_anchor.text` could satisfy its own anchor): fixed by masking the inline block from the anchor corpus length-preservingly (no-op when absent). Retired the obsolete comments-drift pre-commit hook (sub-hook + sourcing block + `assert_comments_drift_hook.sh`; its installer was already deleted in v2.58.0). All 5 resolver node tests + the schema-refuse shell test migrated to inline fixtures; full comments suite + coverage gate green. **Also fixed the two pre-existing browser-side stale tests from the same v2.58.0 inline migration (user follow-up):** `comments-js.test.js` — 9 cases rewritten to the current contract (FSA `writeSidecar`/`submitThread`/`triggerSidecarDownload`/localStorage-draft → `postSubmit` POST `/save` 200-success + 409-conflict-banner, `detectMode` read-only/read-write/file://, in-memory review-mode toggle per D14; one honest test pins the retired surface gone; the hand-rolled runner was made **sequential** to stop async/sync global interleaving; the DOM stub gained `className`↔`classList` sync to match the real DOM) → **26/0**; `template_comments_bake.test.sh` — Part A now renders through `render.js` and asserts the **inlined** css/js + `pmos-comments` seed block + `pmos:skill` meta (the old linked-`comments.css?v=` assertion predated the T1 inlining at commit `8930adc`). |
