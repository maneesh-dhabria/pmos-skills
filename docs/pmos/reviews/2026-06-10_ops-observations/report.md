...
4. **Backlog loops: design finalized same day, after discussion — to be implemented as a separate piece of work** (own `/skill-sdlc` run; deliberately NOT part of this review's apply wave). The discussion evolved 05's two-loop proposal into a **three-loop model** (define / build / release) with a mandatory epic → story → task hierarchy: story is the atomic backlog unit (one story = one worktree = one plan), requirements+spec run at epic level with /plan per story, tasks live in a derived `tasks.yaml` per story (single home: /plan emits, /execute updates, /backlog renders read-only), `next`+`claim` picker at story level, and `/complete-dev --epic` as the release train. Full design with schemas, loop contracts, per-skill change list, migration, and risks: [backlog-three-loop-design.md](backlog-three-loop-design.md). [05](05_backlog-ideation-loop.md) remains the gap-analysis input.
