...
- **T4 (Phase Resolve, F1/D4)** — resolve the candidate brief through the existing reference-resolution precedence (`--brief` → round → role default), made available to Score as the bucketing baseline; absent brief → fourth bucket degrades, never fabricated. No new plumbing (D4).
