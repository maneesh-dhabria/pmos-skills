...
## 2026-07-21 — pmos-managerkit 0.8.0: interview scoring now has a written bar, and the score has to defend itself

**`/interview-guide` now authors the bar, and `/interview-feedback` scores against it.** Every scale option on a generated scorecard carries a level descriptor — behavioural wording that says what a candidate at *that* level actually did, per dimension, so "3 out of 5" means something a second reader can check. The 8 bundled archetype scorecards were backfilled with 188 of these descriptors, so the calibration applies on the default paths, not just on freshly generated sheets. Descriptors are all-or-none within a dimension: a half-described scale is exactly the ambiguity that invites borrowing the top level's standard down to the pass line, and a validator refuses it. Where a sheet's dimension has no descriptors at all, the round no longer gets scored against a bar that exists only in the scorer's head — the bar is derived from the round's own materials, shown to you for agreement, and written into the sheet so the next run inherits it.

**Scoring itself changed shape: sweep the whole transcript first, then conclude.** The old failure mode was scoring a candidate on where they were early in the round — a competency they eventually demonstrated after a probe was being counted against them as absent. Every dimension now collects its evidence across the entire transcript before a score is written, renders that sweep inline (collapsed) so you can see what was weighed, and every below-bar call has to answer "what is the strongest case this was actually fine?" in writing. Four blocking gates enforce it, and all the arithmetic — modal score, weighted score, recommendation band — is computed by a script rather than by hand. A competency the round never probed is now tagged untested and leaves the weighted score entirely, with the remaining weights renormalized and a coverage figure reported, instead of being quietly scored "neutral".

**Artifacts can no longer be handed over half-empty.** A completeness gate refuses to let either output be declared done while a promised section is still unfilled — the observer's independent read block, which had shipped empty before, is the case in point — and stamps a visible `draft — pending` marker when one is. The proof-of-pass comment on a passing citation check is now written by the script that did the checking, so the count in the artifact can't drift from the count that was actually verified.

**Skill quality & internals:** the `data-level` contract gained an explicit bundled-corpus vs per-round scope so the authoring and scoring skills can't drift apart; rubric materialization triggers per dimension rather than per sheet (a partially-backfilled sheet is the expected state); the citation and completeness gates now exit cleanly with the offending path instead of throwing on an unreadable input; `/interview-feedback`'s coach phase gained a documented path for rounds with no transcript; and the README now lists all three manager skills.

**References:**
- [`docs/pmos/features/2026-07-21_interview-scoring-calibration/02_design.html`](features/2026-07-21_interview-scoring-calibration/02_design.html)
...
**The downstream consumers were re-pointed to the new SVG contract in the same cutover.** `/prototype` reads the screen manifest to seed its mock data, `/msf-wf` scores against the rewritten SVG-native rubric, `/design-crit` now recognises an SVG wireframe and skips its raster slop-prepass instead of misreading it, and the inline-comment write-back path learned `/diagram`'s data-anchor resolution so comments on an SVG wireframe still land. **Skill quality & internals:** the evaluation rubric was rewritten as an SVG-native, judgment-only set (the old A1–A5 / D1–D4 heuristics retired in favour of an enumerated survivor id-set with a zero-dangling-cite gate); the HTML template, house-style table, and canvas viewer were reconciled to the monochrome inline-SVG payload; and `--fidelity` is now parsed as natural-language sugar rather than a contract flag.
...
**The rest of the document got the same treatment.** Success metrics are now asked question-first across Dutta Doshi's six categories rather than picked because they were easy to instrument. A dedicated Motivation / Friction / Satisfaction section makes you narrate the user's experience of the change, not just its shape. Every user story carries its own validation plan. And §11 opens with a pre-mortem — imagine it failed, say why — plus an AI-specific risk pass that only fires when the feature actually involves a model. Alongside the template, `/artifact` now offers to turn a finished PRD into conceptual wireframes: after the draft settles it can hand off to `/wireframes` (default) or `/prototype`, and fold the result back into §6 in place of the wireframe placeholder.

## 2026-07-09 — pmos-managerkit 0.6.0: run a work-history round — chronological deep-dive, scored against a level ladder

**`/interview-guide` and `/interview-feedback` now support `work-history`, a structured chronological deep-dive through a candidate's actual career, as a first-class round archetype — the eighth bundled type.** Unlike the case-style rounds, it emits no business case: you walk each role in turn and interrogate what the candidate actually owned. The guide hands you a filled interviewer reference and a scorecard carrying four candidate-blind per-role evidence blocks — company, title, tenure, scope, contribution, result — each with an explicit *was the result measured?* verdict, plus a trajectory-synthesis block that reads the arc across roles: how scope grew, what patterns repeat, and whether the whole record fits the level you're hiring for.

**Its weights come from a level ladder rather than free assignment.** Because a "strong" signal at senior is a different bar than at staff, `--seniority` selects a pre-summed weight row across the twelve competency dimensions, and `--level-rubric` overrides it for teams with their own ladder; the arithmetic is owned by a gate, never totted up by hand. `/interview-feedback` fills the round's per-role and trajectory blocks alongside the usual competency scoring, and scores the level verdict as its own input feeding the recommendation rather than deriving it from one. The new pass is inert on every other archetype — the seven existing round types produce byte-identical output — and every subjective per-role note carries the same transcript-grounded citation the competency notes do.

## 2026-07-09 — pmos-managerkit 0.5.0: /interview-feedback only stands behind evidence it can prove

**`/interview-feedback` now refuses to hand you a scorecard whose quotes don't survive a check against the transcript.** Citation verification is a blocking gate rather than an afterthought: the deliverable cannot be presented as done while any quote fails to match, and a quote can no longer be quietly stitched together across two different speakers' lines — so the sentence you read attributed to a candidate is a sentence that candidate actually said, contiguously. The skill also extracts its quotes the same normalized way the checker compares them, which removes a whole class of phantom "failed citation" that was really just a line break.

Two scoring corrections land alongside it. When a candidate's take-home brief already publishes a structure, that structure is now scored as a **neutral expected baseline** — a fourth submission bucket sitting beside *discussed*, *interviewer-directed*, and *independent* — instead of being misread as the interviewer having seeded the answer, which had been inverting the finding on candidates who simply followed the brief they were given. And time-sensitive dimensions (coverage, talk-time, pace) are now scored against the round's *confirmed* duration rather than whatever the scorecard header claimed, with a transcript-length mismatch surfaced rather than silently absorbed. Together these remove the three redo cycles a real two-candidate run needed.

## 2026-07-06 — pmos-toolkit 2.103.0: new skill — /wayrttd, a 2-minute "am I solving the right thing?" gut-check

**`/wayrttd` ("What Are You Really Trying To Do?") is a fast, reflexive check that catches you before you build the wrong thing.** When you catch yourself reaching for a solution, run it: it takes the solution you're assuming, climbs the ladder from it to the actual intent behind it, names the real problem in your own first-person words, then re-tests your original solution against that real problem and gives you a verdict — proceed, reconsider, or pivot — with a compact captured record. It's deliberately the 2-minute counterpart to the deeper `/shape`: where `/shape` runs a full problem-shaping session, `/wayrttd` is the quick inversion you do on the spot. And it now plugs into `/shape` as an optional upfront pre-check — when you start a shaping session, `/shape` can first run the `/wayrttd` gut-check and carry the real problem it surfaces forward as the thing to shape, so you shape the right problem instead of the one you walked in assuming. The pre-check is fully skippable, and a standalone `/shape` behaves exactly as before when you decline it or don't have `/wayrttd` installed.

## 2026-07-06 — pmos-toolkit 2.102.0: /mytasks tracks durable goals with dated milestones

**`/mytasks` can now hold long-horizon goals so they stop going invisible between the daily task churn.** A goal is a durable object with dated milestones, a cadence, and a type — either dated (with a target) or open-ended. You attach work to a goal in two ways: point a whole project at a goal and every task in it inherits that goal automatically, or set a goal (and milestone) directly on a single task, which always wins over the inherited one. Because goals are easy to set and then forget, `/mytasks` now surfaces the two ways a goal quietly dies: it flags a goal as **behind** when you're off pace against its milestone dates, and as **starved** when it hasn't seen real progress in a while — and it only counts genuine progress, so a trivial edit never masks a stalled goal, and an open-ended goal is never wrongly called "behind". Goals show up in a dedicated goals view, a summary line that stays quiet while everything's healthy, a read-only web view, and a "Goals needing attention" lane in your `/morning-brief` — so the goals that need you are the ones you see.
