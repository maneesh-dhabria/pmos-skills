...
3. **Platform + gates inventory** — Codex-only tool dependencies and whether degradation notes are followable; every rubric/gate/lint with hard-vs-soft verdicts; **run the repo's own audit scripts and report whether they actually pass** — a red gate everyone ignores is itself a top finding.
