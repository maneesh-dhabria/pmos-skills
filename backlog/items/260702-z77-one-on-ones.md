...
New pmos-managerkit skill `/one-on-one` (charter: help me do manager work). It helps a manager run **effective**
one-on-one with their direct reports — not merely organize notes. Shaped via `/shape`
...
**Independence (user constraint):** `/one-on-one` is **fully independent of `/interview-feedback`** — no interop,
...
- [ ] A manager can **add a direct report** (`/one-on-one add <name>`): the skill ensures a `/people` identity
...
- [ ] A manager can **quick-capture a discussion item** in one command
  (`/one-on-one note <report> "<item>"`) that appends to that report's running-agenda inbox and seeds the next
  session — zero-friction, `/backlog add`-style.
- [ ] A manager can **plan/prep a 1:1** (`/one-on-one plan <report>`): the skill assembles a human-first agenda
...
- [ ] A manager can **log a session** (`/one-on-one log <report>`): capture topics, decisions, action items
...
- [ ] A **periodic career conversation** (`/one-on-one career <report>`, Laraway's 3-part model) is offered
...
- **260702-6ks** — the `/one-on-one` skill end-to-end: data model + record parser/serializer, the five verbs
...
- README row for `/one-on-one`; changelog entry; manifest version-sync.
