...
- [x] **Theme registry (7 backgrounds)** — a pure data registry of 7 themes (`sky` default, `ocean`,
...
- [x] **4 procedural bird shapes** — `round` (classic), `sparrow` (slim), `duck` (broad flat bill), `owl`
...
- [x] **5-color contrast-safe palette** — a fixed palette of 5 colors, each `{body, accent, beak, lum}`
...
- [x] **Pure seedable `pickVariants(seed, prevBg)`** — on the engine surface (alongside `FlappyEngine`),
...
- [x] **Subtle top-HUD pmos wordmark** — a translucent, unobtrusive `pmos · flappy-bird` element added to
...
- [x] **Selftests (`tests/run.mjs`) extended, all green** — assert: (a) registry completeness (7 themes ×
...
- [x] **Live headless dogfood** — the game boots, renders a non-default theme + a non-yellow/non-round bird
...
- [x] **Gameplay preserved + hygiene** — physics/scoring/difficulty/pause/ceiling-clamp/session-best
...
Built on `feat/260624-vry` (build commit `713f00c2`), branched off main, no deps. All changes
land in `game/flappy-bird.html` (engine + guarded render) + `tests/run.mjs`; SKILL.md got a
one-sentence launch-paragraph touch (launch contract untouched). Inv-1 held — registries +
`pickVariants(seed, prevBg)` are pure/seedable/selftested; only consumption is render-side.

**Gates (all green):**
- Selftest `tests/run.mjs` — 65/65 (47 pre-existing physics/scoring untouched + 18 new:
  8 registry-completeness + 10 picker invariants I-contrast/I-repeat/I-determinism/I-coverage);
  `--selftest` count-gate exit 0.
- `skill-eval` `[D]` — all checks pass, exit 0, no new residuals (SKILL.md prose-only change).
- 4 hygiene lints — `lint-non-interactive-inline` (52 skills PASS), `lint-flags-vs-hints` PASS,
  `lint-phase-refs` PASS, `audit-recommended` PASS (0 calls — prompt-free skill).
- Live headless dogfood (Playwright, `http://localhost:8731`) — `pickVariants(2,0)` →
  forest/sparrow/sky-blue render (non-default theme, non-round bird, non-yellow color), sky pixel
  `rgba(197,228,166)` non-blank; `#wordmark` href == repo URL, target `_blank`, rel `noopener`,
  text `pmos · flappy-bird`; flap→score 3→game-over→restart (fresh `ocean`, score 0) cycle works;
  console 0 errors / 0 warnings. Evidence: `dogfood/dogfood-run.md` + `dogfood/flappy-forest-sparrow-skyblue.png`.

Worktree KEPT for Loop-3. Next: `/complete-dev --epic 260624-fly` (pmos-gamekit minor bump).
