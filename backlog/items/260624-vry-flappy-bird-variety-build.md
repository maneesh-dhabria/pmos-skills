...
- [ ] **Theme registry (7 backgrounds)** — a pure data registry of 7 themes (`sky` default, `ocean`,
...
- [ ] **4 procedural bird shapes** — `round` (classic), `sparrow` (slim), `duck` (broad flat bill), `owl`
...
- [ ] **5-color contrast-safe palette** — a fixed palette of 5 colors, each `{body, accent, beak, lum}`
...
- [ ] **Pure seedable `pickVariants(seed, prevBg)`** — on the engine surface (alongside `FlappyEngine`),
...
- [ ] **Subtle top-HUD pmos wordmark** — a translucent, unobtrusive `pmos · flappy-bird` element added to
...
- [ ] **Selftests (`tests/run.mjs`) extended, all green** — assert: (a) registry completeness (7 themes ×
...
- [ ] **Live headless dogfood** — the game boots, renders a non-default theme + a non-yellow/non-round bird
...
- [ ] **Gameplay preserved + hygiene** — physics/scoring/difficulty/pause/ceiling-clamp/session-best
