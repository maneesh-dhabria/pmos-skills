...
- [ ] **AC1 — Canonical format + validator (single home, §K).** `_shared/tracker-crudl.md` §2.1
...
- [ ] **AC2 — Year-prefixed minter (D1).** `backlog/scripts/mint-id.mjs` `mintId()` prepends the
...
- [ ] **AC3 — Triple-accept, no migration (D1).** The validator accepts all three forms; every
...
- [ ] **AC4 — /mytasks adopts the scheme (D2/D3, veto-able).** `mytasks/SKILL.md` quick-capture
...
- [ ] **AC5 — Tests + prose (regression guard).** `backlog/tests/id-scheme.test.sh` expects a 6-digit
