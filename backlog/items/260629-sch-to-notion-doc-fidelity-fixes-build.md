...
- [x] **AC1 (F1)** Phase 5 always emits a Post-conversion actions section: (a) count of in-page anchor links
  ...
  plan, not hard-coded.
- [x] **AC2 (F2)** `map-to-notion.mjs` emits exactly one stub callout per image node and per ambiguous-media
...
- [x] **AC3 (F3/F4)** `upload-image.mjs` `buildStub()` returns a single callout
...
- [x] **AC4 (F5)** `parse-doc.mjs` detects `<dl>/<dt>/<dd>` and clean alternating label→description div
...
- [x] **AC5 (F6)** `parse-doc.mjs` detects ≥3 sibling top-level sections sharing a leading token
...
- [x] **AC6 (F7)** Each phase ends with a one-line compaction-surviving banner to chat
  (`Phase N complete — <counts>. Phase N+1 starting.`).
- [x] **AC7 (no-regression)** A source with no images / no dl-or-label-divs / no annexure clusters converts
  ...
  (INV-5).
- [x] **AC8 (conformance)** Conforms to `skill-patterns.md §A–§L`; passes `skill-eval.md` (`[D]`+`[J]`); 4
