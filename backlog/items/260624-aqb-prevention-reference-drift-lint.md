...
1. `_shared/slop-engine/design-slop-rules.md` (the prevention reference) is **generated from the
...
2. The generated floor is **cited (one-line, per §K) as a design floor** in all three UI-generating
...
3. `tools/lint-slop-rules.sh` (repo-root, bash-3.2-safe) asserts every `SLOP_RULES` entry with a
...
4. The lint is wired into `.github/workflows/skill-hygiene.yml` alongside the existing lints; a
   deliberately-broken fixture proves it fails.
5. Inv-3 holds: a grep for `impeccable` over the new/edited files returns hits only in design-doc
   lineage prose / the engine `NOTICE` (not introduced by this story).
6. The three SKILL.md edits conform to `skill-patterns.md §A–§L`; `skill-eval` passes for each touched
   skill; 4 lints + audit clean; no release-prereq tasks in the plan.
7. Live dogfood: edit a rule's `skillGuideline` without updating the floor → the lint fails; regenerate
