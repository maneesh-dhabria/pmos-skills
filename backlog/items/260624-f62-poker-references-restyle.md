...
- [ ] **Collapsible References panel (D1, §2)** — a two-column shell (table left, ~340px panel right) with a
...
- [ ] **Hand rankings reference (§3.1)** — all 9 categories strongest-first with examples, rendered from
   bundled data; static; suit glyphs colored consistently with the table cards.
- [ ] **Interactive preflop chart (§3.2)** — 13×13 grid of 169 hands, Position × Facing selectors, cells
...
- [ ] **Bundled static data + scope discipline (D6, §3.2)** — the 9 ranking rows, per-position open ranges,
...
- [ ] **Pot-odds + Rule-of-2&4 card (§3.3)** — a static card covering counting outs, Rule of 2 & 4 (+big-draw
...
- [ ] **Table restyle (D2, §4)** — oval felt with hero bottom-center, seat tiles with per-street chip strips,
...
- [ ] **Pure, self-tested engine (§6)** — `handKey()` / `chartAction(cards, position, facing)` /
...
- [ ] **HUD "Poker" title + pmos watermark (D7, §5)** — the in-game HUD heading reads "Poker" (user-facing
...
- [ ] **Gates green + dogfood** — `node tests/run.mjs` exits 0; grep-assert no coaching/equity/EV/persistence/
