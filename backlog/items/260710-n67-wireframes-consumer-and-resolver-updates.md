...
- [ ] **[Blocker] The apply-edit shim resolves `data-anchor`.** `_resolveAnchor` gains the `data-anchor="<id>"`
...
- [ ] A **new fixture is an inline-`<svg>` screen** in the post-refactor emit format, and a test asserts a comment
...
- [ ] **`/prototype` reads the manifest first.** `SKILL.md:183` and `:219`, and `reference/mock-data-prompt.md:8–10`,
...
- [ ] The entity model can no longer come back **silently empty**: when neither the manifest nor the legacy tags
...
- [ ] **`/design-crit`'s SVG blindness is resolved explicitly.** `window.pmosDesignScan()` — which lives at
...
- [ ] **`build-canvas.js` canvas tokens reconciled** (amendment A8). `SCREEN_WIDTH`/`SCREEN_HEIGHT`
...
- [ ] **`/msf-wf`'s rubric cite stays valid.** `msf-wf/SKILL.md:185` spot-checks edits against
...
- [ ] `/msf-wf --apply-edits` (the block at `msf-wf/SKILL.md:181–188`) still lands substring `Edit`s against the SVG
  payload; a fixture test covers a coordinate-adjacent edit (design §7 risk 3).
- [ ] `plugins/pmos-toolkit/README.md:25` no longer describes the output as "Static mid-fi HTML wireframes".
- [ ] `check-comments-coverage.sh` passes; its contract-test and emit-reference inventory is unchanged by this
