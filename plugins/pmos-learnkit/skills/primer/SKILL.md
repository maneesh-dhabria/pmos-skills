...
These are SOFT targets — the Phase-5 reviewer (when triggered) reports actual `word_count` on the R10 object as an informational field; there is NO hard-block on miss. Resolved depth comes from Phase 0.5 resolution (`cli > settings > lastrun > prompt`). The previous "default ~3000 words" sizing is now `depth=brief|standard` territory; `deep` is new in v0.2.0 and unlocks the 7K–10K range for broad senior-PM topics.
...
**Non-goals (spec §3).** No separate diagram-pass — diagrams ship inline during the single-pass draft when a visual aids comprehension (see "Inline SVG diagrams" above). No persona-pass. No blind-primer. Single linear draft against the outline; the (opt-in) reviewer enters in Phase 5.
...
**Reviewer trigger (opt-in).** Steps 1–6 run only at `--depth deep` or when the user explicitly asks for a review pass. At `brief`/`standard`, skip directly to step 7 — the draft-time trust checks (FR-7.1 citation discipline, Phase-3 grounded takeaways, the audience-preset required-sections contract, step-11 `sections.json` construction) already enforce the same trust invariants at construction time, and observed reviewer runs at these depths returned all-pass with zero findings every time. When the reviewer is skipped, step 7's R10-derived informational notes are omitted and its question text reads `"Draft ready (reviewer skipped at this depth). Approve, iterate, or abort?"`.
...
5. **Trust-tier hard-block (FR-8.4).** After iteration 1's re-run, if ANY trust-tier check is still failing (R1 `cites-real-urls`, R2 `no-plagiarism`, R3 `no-hand-wavy-claims`, R6 `structurally-complete`, R7 `sections-json-ids-match`), execute the FR-RECOVERY path (step 16 below) and exit.
...
- Do NOT invent URLs that do not appear in `sources.json` — citation discipline is the trust-tier hard-block (FR-7.1, rubric R1). The reviewer (when triggered) cross-checks every `<a href>` against `sources.json[].url`; mismatches trigger FR-RECOVERY.
- Do NOT skip the Phase-5 reviewer once triggered (`--depth deep` or a user-requested review pass) — there is no clean exit path that bypasses a triggered reviewer (FR-8 + FR-8.4); the artifact then ships only after the reviewer has scored the draft and the orchestrator has validated the JSON. At `brief`/`standard` the reviewer is opt-in and the draft-time trust checks are the quality bar.
