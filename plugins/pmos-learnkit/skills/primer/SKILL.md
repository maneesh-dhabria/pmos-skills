...
user-invocable: true
argument-hint: "[<topic> | browse | list | enrich] [--audience <senior-pms|all-pms>] [--depth <brief|standard|deep>] [--autonomous] [--non-interactive] [--interactive]"
---
...
**Verb dispatch (before anything else).** If the first token is exactly `browse` or `list`, **or** the invocation carries no topic at all (bare `/primer` with only flags or nothing), this is a library request, not a research run: go straight to the **Browse the library** phase ([`#browse`](#browse)) and skip Phases 2–6 entirely. If the first token is exactly `enrich` (natural language: "enrich the bundled primers with curated sources"), this is the retrospective corpus-refine path: go to **Enrich the corpus** ([`#enrich`](#enrich)) and skip the research phases. Otherwise the first token is the `<topic>` and the research pipeline below runs. **The forward new-primer generate path (Phases 2–5) and `--no-curated` semantics are UNCHANGED by the enrich verb (D8)** — enrichment is additive tooling over already-shipped primers, not a change to how new primers are made.
...
## Enrich the corpus {#enrich}

Reached from the Phase-1 verb dispatch (`enrich`). A **retrospective, corpus-aware refine** of already-shipped primers: the 61 bundled primers were generated before the curated corpus reached today's ~1,800 records, so their evidence sets under-use it. This verb runs the existing curated-references overlay prefilter over each primer's own topics, **fetch-verifies the top curated candidates THIS run**, and — for survivors — weaves them into the matching teaching section's prose and `sources.json`, then regenerates `## References`. Not a References-only append (D1) — a real rebuild-on-the-enriched-set, so `## References`' "sources this primer was built on" semantics stay honest.

The deterministic engine is `scripts/enrich-references.mjs` (zero-dep Node ESM; the sole author of this behavior). It exposes `enrichPrimer({htmlPath, sourcesPath, corpus, tagVocabulary, tagSynonyms, opts, deps})` plus the pieces (`recoverTopics`, `prefilterCandidates`, `defaultWeaver`, `deterministicRubric`). **The two non-deterministic parts — fetch-verify (network) and the optional richer LLM prose weave — are injected via `deps`**, so the deterministic core (prefilter, dedup, coverage gate, cap, additive weave, rubric gate, atomic write/revert) is testable and the backfill runs lights-out.

Per-primer procedure (design §4.1, INV-1…INV-6):

1. **Prefilter (deterministic, INV-1/INV-5).** `enrichPrimer(..., opts:{dryRun:true})` (or the CLI `--dry-run`) recovers the primer's topics from `sources.json[].topic`, picks query tags from the corpus tag-vocabulary, runs the **existing** `curated-references-match.mjs` (never a second relevance mechanism), **excludes** candidates already in the primer's `sources.json`, and applies the overlay coverage gate (`T=3`, `S=0`). A primer with every topic skipped is a logged no-op.
2. **Fetch-verify (INV-2/D4).** Route each surviving candidate through `_shared/topic-research/sourcing.md`'s hard-gate + fetch-verify loop **this run** (WebFetch), treating curated survivors identically to live-search candidates; drop failures (~30% yield). Cap admitted new sources per primer (`--cap`, default ~5). Supply the verified records to the engine as `deps.verify`. **No corpus record is written on trust.** WebFetch unavailable → degrade per `reference/source-floor.md`: verify nothing, add nothing, log the degraded run (never admit unverified sources).
3. **Weave + regenerate + gate (INV-3/INV-4/INV-6/D5/D7).** The engine weaves each verified source additively into the H2 whose `id` matches the source's `topic` (never rewriting or deleting existing prose), appends the `{url, takeaway, topic, tier, paywalled}` entry to `sources.json`, regenerates `## References` via 260704-rgt's `injectReferences` (never hand-authored), re-scores against `reference/rubric.md` (R1 citation-membership + R11 References + trust-tier), and **writes atomically on pass / reverts the primer to its pre-enrichment bytes on fail**. A primer that fails the rubric after enrichment is never shipped.

Re-runnable + convergent (INV-5): a second run over an already-enriched primer adds nothing — the cap is a **per-primer total** (counted from the woven `<p class="primer-enriched">` paragraphs already present), so a primer at the cap is a byte-identical no-op rather than accreting the next batch of candidates every run.

The lights-out corpus backfill over all 61 bundled primers is `scripts/backfill-enrichment.mjs` (story `260704-e3b`) — it **imports** `enrichPrimer` (never re-implements it), injecting a Node-native `verify` that re-fetches each curated candidate THIS run purely as a **reachability / link-rot guard** (the corpus records were already hard-gated + tier-assessed at harvest; the grounded `summary` is the takeaway, tier is a domain/`source_type` heuristic) — deliberately weaker than an interactive `enrich` run's full LLM pass-bar, which is documented in its run report. After enrichment it refreshes only `sources_count` in `data/primers-index.json` (a pure function of each `sources.json`, so the refresh is convergent); `word_count` is left untouched (References is excluded from it by definition — design D8) and the regenerable `library.html` is never touched. Run: `node scripts/backfill-enrichment.mjs [--dry-run] [--limit N] [--cap N]`.
...
- **References (generated, NOT drafted).** The primer's final section — `## References`, after `## Where this connects` — is produced **deterministically** from `sources.json` by `scripts/references-section.mjs`, not hand-written (INV-2, mirrors Anti-pattern #9 / #10). Do NOT author a References list in the draft prose: Phase 5 step 7 appends it. The model authors teaching prose + `## Where this connects` only.
...
2. **Orchestrator-side validation** (the orchestrator, not the reviewer): (a) the returned `check_id` set MUST equal rubric.md's 11 IDs (R1–R11; R11 is the deterministic References membership/structure check) — on mismatch, hard-fail with the precise diff; (b) every `fail` quote MUST be a ≥40-char verbatim substring of the draft (or of `sources.json[].takeaway` for R2) — a fail whose quote misses is treated as `pass` (defense against hallucinated quotes). R11 is deterministic (no re-fetch): it runs against the assembled `sources.json` + rendered body + `sections.json`.
...
7. **Write (atomic trio).** On approve, write `{date}_{slug}.html`, `.sections.json`, and `.sources.json` via temp-then-rename — all three temps succeed before any `mv`; on any rename failure, `rm` the temps and abort (no partial states).
   ...
   **Append the References section (deterministic, INV-1/INV-2).** After the content-only `{{content}}` fragment is assembled and **before** `renderArtifact()` runs, generate the References block from the assembled `sources.json` and append it as the **last** content block (after `## Where this connects`, D7). Use the shared generator — never hand-author the list (Anti-pattern #10); this keeps the single-render-path contract (Anti-pattern #9) intact, since References rides `{{content}}` and the shell still comes only from `template.html`:

   ```js
   const { renderReferencesFragment, computeCitedUrls } =
     await import('${CLAUDE_SKILL_DIR}/scripts/references-section.mjs');
   const citedUrls = computeCitedUrls(content, sources);            // which sources are cited inline (D3)
   content = content + '\n' + renderReferencesFragment(sources, citedUrls);  // last block, before render
   ```

   Then add the `references` id to the atomic-trio `sections.json` (its id set MUST equal the on-page id set — R7; `references` is the last entry, in document order). R11 (below) re-checks this deterministically.
...
10. **Hand-authoring the `## References` list instead of generating it from `sources.json`.** The References section is a pure transform of the primer's own `sources.json` (INV-2) — one generator, two callers (`scripts/references-section.mjs`, INV-1). Writing a References list into the draft prose (or omitting a source, or claiming a source was cited that wasn't) drops the honesty guarantee (INV-5) the deterministic path enforces. The model authors teaching prose + `## Where this connects`; Phase 5 step 7 appends References via `renderReferencesFragment()`, and rubric R11 fails the run if a source is missing.
11. **Adding a curated source without fetch-verifying it this run (enrich verb).** A curated-corpus record is a *candidate*, not a verified source — the corpus can carry stale or link-rotted URLs. The `/primer enrich` path ([`#enrich`](#enrich)) admits a curated source only after it passes `sourcing.md`'s hard-gate + fetch-verify **this run** (INV-2, mirrors Anti-pattern #1); unverified candidates are dropped, and when WebFetch is unavailable enrichment adds nothing rather than trusting the corpus. Weaving a corpus URL into prose or `sources.json` on trust is the same trust violation as inventing one.
...
- **Phase 4.** ~4,800-word curator-voiced draft; 14 citations, all verbatim `sources.json[].url` members; two inline SVGs (a rollout-lifecycle flow, a build-vs-buy 2×2) per `reference/diagram-style.md`; closes with `## Where this connects` (3 pointers). The `## References` section is NOT drafted here — Phase 5 generates it.
- **Phase 5.** Before `renderArtifact()`, `renderReferencesFragment(sources, citedUrls)` appends the `## References` section (18 sources: the 14 cited ones marked `·cited·`, the 4 background ones listed too — INV-5), tier-ordered, as the last block; `references` joins the `sections.json` id set; R11 passes deterministically.
