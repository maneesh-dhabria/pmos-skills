...
- **Informational fields.** The R10 object additionally returns three fields used by the orchestrator at the Phase-5 write gate; none affect the R10 verdict:
  ...
  - `word_count: <int>` — actual draft word count. The orchestrator surfaces a one-line note at the write gate when this falls outside the resolved depth tier's target range (brief 2,000–3,000 / standard 4,000–6,000 / deep 7,000–10,000). **`word_count` excludes text inside `<svg>` subtrees** — depth-tier thresholds compare against prose only, so inline diagrams do not inflate the count. **It also excludes the `## References` section** (D8): References is back-matter apparatus, not teaching prose, so the metric stays comparable to pre-References-epic values.
...
- **Tier:** trust
- **Pass criterion:** the deterministic References section is present and honest (INV-5) — all three hold: (a) a `<h2 id="references">` section exists in the rendered primer; (b) **every** `sources.json[].url` appears in the References list (membership — no source silently dropped); (c) the `references` id is present in `sections.json`. Any one failing = fail.
- **Why:** the References section is generated by `scripts/references-section.mjs` from `sources.json` (INV-1/INV-2); R11 guards against a stale, hand-edited, or missing section — the evidence base a reader trusts must reflect every source the primer was built on.
- **How to verify (no re-fetch):** a membership/structure test computed from the assembled `sources.json`, the rendered body, and `sections.json` — like R7, it runs deterministically at construction time and requires no network. Set-difference `sources.json[].url` against the anchors inside the `#references` section; check the `<h2 id="references">` tag and the `references` id in `sections.json`.
- **Evidence shape:** the missing element — a `sources.json` url absent from the list, the absent `<h2 id="references">`, or the missing `references` id in `sections.json`.
- **Quote requirement:** ≥40-char verbatim substring — of the References section when present (showing the gap), or of the `</main>`-adjacent region when the section is absent.
- **Fail example:** `sources.json` has 18 entries but the References `<ol>` lists only 14 (the 4 uncited background sources were dropped) — INV-5 violation.

## Output contract

- Reviewer returns one JSON array, exactly 11 objects, one per check_id above (R1–R11). The check_id set must equal the rubric's set — the orchestrator hard-fails on mismatch.
...
- Trust-tier fails (R1, R2, R3, R6, R7, R11) hard-fail the run and require a re-draft. Taste-tier fails (R4, R5, R8, R9, R10) surface as known-risk and gate on explicit user override. R11 is deterministic (a membership/structure test like R7) — it does not require re-fetching and is safe to verify from the assembled inputs alone.
