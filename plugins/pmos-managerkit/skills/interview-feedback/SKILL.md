...
**What a green pair does not prove (named residuals, not silent ones).** Between them the gates prove each cited quote is *real* and each number is *defended* — they do not prove the sweep was **complete** (a presence check cannot tell one representative instance from the only one you looked at), and they do not prove a quote is **relevant** to the claim built on it (a genuine transcript span can sit under invented interpretive prose). Both are judgement, not arithmetic, so per §H they stay with the method above and with the reviewer — a passing gate is a floor, never a sign-off.
