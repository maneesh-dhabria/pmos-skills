...
**Level-descriptor anchors — author the bar.** On the same pass that assigns `data-weight` (and `data-budget`), emit `data-level="<descriptor>"` on **every** `data-v` option of **every** dimension: that level's at-bar wording for that competency, drawn from the archetype's own strong/weak markers (Phase [Ground](#ground)) calibrated to the seniority resolved in Phase [Collect](#collect) step 4. The sheet is where the bar lives, so `/interview-feedback` can quote the round's own level-3 text instead of inventing one. The attribute's rules — per-dimension, all-or-none, non-empty, behavioural, and *unprompted language is top-level only* — are defined once in `../_shared/interview-guidelines/scorecard-skeleton.html`'s contract comment (§K); read them there rather than re-deriving them. The one that most often goes wrong at this step: **the pass line permits prompting** — a candidate who gets there after a probe is at the bar, so "unprompted"/"up front" wording belongs to the top level only.
...
It asserts every required anchor is present and that the weights are integers summing to 100; it enforces the skeleton's `data-level` contract on any dimension that carries descriptors; **when the time-budget anchors are present** it additionally asserts `data-duration` is a positive integer and the per-dim `data-budget` values are positive integers summing to ≤ `data-duration` (§H: the script owns all of this arithmetic — never total the weights or the budgets by hand). A non-zero exit **blocks the run** — fix the sheet and re-run until it passes; surface the script's `✓ scorecard anchors:` line to the operator on pass. (`--selftest` exercises the validator over good + broken fixtures for the weight sum, the anchor set, the budget overrun, and the non-integer duration.)
