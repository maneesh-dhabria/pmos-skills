---
name: solitaire
description: Play Klondike solitaire in your browser. Use when the user wants to play solitaire or Klondike, take a break with a card game, or says "/solitaire", "play solitaire", "deal me a game of solitaire", "I want to play cards". Launches a pre-bundled, offline single-player card game via a tiny local server — no setup, no network, no account.
user-invocable: true
argument-hint: "(no arguments — deals a fresh game; draw-1/draw-3, undo, and auto-move are in-game controls)"
...
- **Game file:** `game/solitaire.html` (this skill's directory).
- **Title:** Klondike solitaire.
- **No options to resolve:** draw mode, undo, and auto-move are in-game controls, so the
...
- **No Bash tool:** the user runs the one launch command themselves —
  `node <skill-dir>/../_shared/game-launcher/serve.js <skill-dir>/game/solitaire.html` —
...
Read `~/.pmos/learnings.md` if present and apply any entries under `## /solitaire` to this
...
1. **Resolve paths** relative to this skill's directory (`<skill-dir>`):
   - Game: `<skill-dir>/game/solitaire.html`
...
under `## /solitaire` in `~/.pmos/learnings.md`. Nothing notable → write nothing. Never
