---
name: poker
description: Play No-Limit Texas Hold'em in your browser. Use when the user wants to play poker, play Texas Hold'em, deal a poker hand, play cards against bots, or says "/poker", "play poker", "let's play hold'em", "deal me in". Launches a pre-bundled, offline single-player cash game against heuristic bots via a tiny local server — no setup, no network, no account.
user-invocable: true
argument-hint: "(no arguments — deals a fresh game; opponent count, betting, raise slider, and quick-bets are in-game controls)"
...
self-contained No-Limit Hold'em game (`game/poker.html`) through the shared zero-dependency
launcher and points your browser at it. It is a single-player cash game: you sit against
**1–5 random heuristic bots** (chosen on the start screen, or randomized), post blinds, and
...
- **Game file:** `game/poker.html` (this skill's directory).
- **Title:** No-Limit Texas Hold'em.
- **No options to resolve:** opponent count is chosen on the in-page start screen, and all
  betting (fold / check / call / raise slider / quick-bets / all-in), next-hand, and new-game
  are in-game controls, so the skill issues **no prompts** and needs no arguments.
...
- **No Bash tool:** the user runs the one launch command themselves —
  `node <skill-dir>/../_shared/game-launcher/serve.js <skill-dir>/game/poker.html` —
...
Read `~/.pmos/learnings.md` if present and apply any entries under `## /poker` to this
...
1. **Resolve paths** relative to this skill's directory (`<skill-dir>`):
   - Game: `<skill-dir>/game/poker.html`
...
4. **Report** the URL to the user and that gameplay is: pick the number of opponents
   (**Random** or **1–5**) on the start screen, then **Deal**. Each hand you post blinds and
...
under `## /poker` in `~/.pmos/learnings.md`. Nothing notable → write nothing. Never
record game state — there is none to keep (D7).
