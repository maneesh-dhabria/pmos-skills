---
name: "2048"
description: Play 2048 in your browser. Use when the user wants to play 2048, slide tiles to reach 2048, take a break with a number-puzzle game, or says "/2048", "play 2048", "let's play 2048", "I want a tile-sliding puzzle". Launches a pre-bundled, offline single-player game via a tiny local server — no setup, no network, no account.
user-invocable: true
argument-hint: "(no arguments — opens the game; board size 4×4/5×5/6×6, undo, and keep-playing are in-game controls)"
...
self-contained 2048 game (`game/2048.html`) through the shared zero-dependency launcher and
points your browser at it. All gameplay (board-size picker 4×4/5×5/6×6, arrow/WASD/swipe/D-pad
moves, one-step undo, keep-playing past 2048, win + game-over states, live score + session
best + move counter) lives inside the one HTML file and runs offline.
...
- **Game file:** `game/2048.html` (this skill's directory).
- **Title:** 2048.
- **No options to resolve:** board size, undo, and keep-playing are in-game controls, so the
  skill issues **no prompts** and needs no arguments.
...
- **No Bash tool:** the user runs the one launch command themselves —
  `node <skill-dir>/../_shared/game-launcher/serve.js <skill-dir>/game/2048.html` —
...
Read `~/.pmos/learnings.md` if present and apply any entries under `## /2048` to this
...
4. **Report** the URL to the user and that gameplay controls are: pick a board size
   (4×4/5×5/6×6) then **Play**; **arrow keys** or **WASD** or **swipe** or the on-screen
   **D-pad** to slide tiles; **U** undo (one step); **R** or **N** new game; **Esc** closes
   overlays. Reach a 2048 tile to win, then **Keep playing** for a higher score. Stopping is
   `Ctrl-C` in the launcher terminal. Per the launch contract the game keeps **no save
   state** — closing the tab discards the board (session best resets too).
...
under `## /2048` in `~/.pmos/learnings.md`. Nothing notable → write nothing. Never
