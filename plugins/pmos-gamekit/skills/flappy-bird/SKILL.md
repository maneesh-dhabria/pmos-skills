---
name: flappy-bird
description: Play Flappy Bird in your browser. Use when the user wants to play Flappy Bird, flap a bird through pipes, take a break with a one-button arcade game, or says "/flappy-bird", "play flappy bird", "flappy bird game", "I want to play flappy bird", "tap to flap". Launches a pre-bundled, offline single-player game via a tiny local server — no setup, no network, no account.
user-invocable: true
argument-hint: "(no arguments — opens the game; difficulty, flap, pause, and restart are in-game controls)"
...
self-contained, one-button Flappy Bird (`game/flappy-bird.html`) through the shared
...
- **Game file:** `game/flappy-bird.html` (this skill's directory).
- **Title:** Flappy Bird.
- **One-button game:** difficulty (Easy/Normal/Hard) is picked on the start screen; flap,
  pause, and restart are in-game controls, so the skill issues **no prompts** and needs no
  arguments.
...
- **No Bash tool:** the user runs the one launch command themselves —
  `node <skill-dir>/../_shared/game-launcher/serve.js <skill-dir>/game/flappy-bird.html` —
...
Read `~/.pmos/learnings.md` if present and apply any entries under `## /flappy-bird` to this
...
1. **Resolve paths** relative to this skill's directory (`<skill-dir>`):
   - Game: `<skill-dir>/game/flappy-bird.html`
...
4. **Report** the URL to the user and that gameplay controls are: pick a difficulty
   (**Easy**/**Normal**/**Hard**) then **Play**; **Space** / **↑** / **mouse click** /
   **touch tap** all flap; **P** pause; **R**/**N** new game. The first flap starts the run.
   Thread the bird through the gaps in the pipes — each pipe pair cleared scores **+1**; the
   ceiling is safe (the bird can't fly off the top), but the **ground and the pipes are
   lethal**. A game-over overlay shows the final score, the session best, and a restart; a
   new session best fires a small celebratory flourish. Stopping is `Ctrl-C` in the launcher
   terminal. Per the launch contract the game keeps **no save state** — closing the tab
   discards the board (the session best resets too).
...
under `## /flappy-bird` in `~/.pmos/learnings.md`. Nothing notable → write nothing. Never
