#!/usr/bin/env node
// T20 — per-skill contract test for /ideate's "Apply comment-resolver edit" shim.
...
const FIXTURE = path.join(__dirname, "fixtures", "apply-edit-at-anchor", "ideate_mini.html");
...
(async () => {
  ...
  if (failures.length > 0) {
    console.error("FAIL: /ideate apply-edit-at-anchor — " + failures.length + " of 5 cases");
    for (const f of failures) console.error("  - " + f);
    process.exit(1);
  }
  console.log("PASS: /ideate apply-edit-at-anchor — 5 cases");
})().catch((e) => {
