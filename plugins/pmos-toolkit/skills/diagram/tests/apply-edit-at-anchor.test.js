#!/usr/bin/env node
// T19 — per-skill contract test for /diagram "Apply comment-resolver edit" shim.
// Exercises 5 cases against the §9.1 input/output contract from
// plugins/pmos-toolkit/skills/_shared/apply-edit-at-anchor.md.
//
// Diagram-specific infeasible case: geometry edits (move, resize, coords)
// must return agent_judged_infeasible (SVG-retrofit deferred to T23).
// Happy path uses data-anchor= on SVG nodes (diagram-native anchor style).
...
const FIXTURE = path.join(__dirname, "fixtures", "apply-edit-at-anchor", "diagram_mini.html");
  ...
  return Object.assign(
    {
      ...
      thread_id: "T_A",
      anchor: { id_anchor: "diagram-notes", quote_anchor: null },
      body: "tighten the description of the three-tier architecture",
    },
    overrides || {}
  );
}
...
(async () => {
  ...
  // (a) Happy id-first path — T_A (anchors to <section id="diagram-notes">)
  try {
  ...
  try {
    const out = await apply(
      mkInput({
        thread_id: "T_C",
        anchor: {
          id_anchor: "missing-node",
          quote_anchor: { text: "a phrase that does not appear anywhere in the diagram at all xyzzy plugh" },
        },
        body: "rename this node",
      })
    );
  ...
  // (d) agent_judged_infeasible: geometry edit heuristic.
  try {
    const out = await apply(
      mkInput({
        thread_id: "T_D",
        anchor: { id_anchor: "diagram-notes" },
        body: "move the Database node to the left and resize it to be 200px wide",
      })
    );
  ...
  if (failures.length > 0) {
    console.error("FAIL: /diagram apply-edit-at-anchor — " + failures.length + " of 5 cases");
    for (const f of failures) console.error("  - " + f);
    process.exit(1);
  }
  console.log("PASS: /diagram apply-edit-at-anchor — 5 cases");
})().catch((e) => {
