...
**Wireframe exemption (epic 260710-grd A9).** A monochrome-SVG `/wireframes` artifact (detected by its `<meta name="pmos:skill" content="wireframes">` head tag) carries no HTML/CSS surface for the slop tells to read — they are HTML/CSS heuristics, and the payload is SVG primitives. The helper detects this **before** scanning and records an **earned skip** through the same graceful-degradation machinery (the contracted skip-note + `skipped: true` shape; `slop-prepass.mjs` is its home, §K) with a wireframe-specific reason — so the run says it skipped and why, never a silent cap.
