...
- **Proof-of-execution — record the runner's own line, never a narrative claim.** Before you write the Slop-gate verdict you MUST surface, verbatim, the runner's literal output line **for this run**: either its run-summary (`[slop-gate] … GATE FIRES (verdict drops below PASS)` / `… no quality blocker; slop advisory only`) or its skip-note (`[slop-gate] slop gate skipped — engine/parser unavailable: <reason>`). Whether the gate **ran or skipped** is keyed to the runner's JSON report (`ran` / `skipped`) **and** its exit code — never to prose: a skip is recorded **only** when the runner emitted that skip-note (JSON `ran==false` / `skipped==true`, exit `0` via the skip path), and a fire **only** on exit `2`. The full runner-output contract — stdout JSON shape, the two stderr lines, and exit codes `2`/`0`/`64` — has one home in `scripts/slop-gate.mjs`'s header (and epic design §5, `02_design.html#helper-output`); cite it, don't restate it here (§K).
- **Graceful degradation (Inv-5) — never flips a correct PASS to FAIL on tooling absence; degradation is earned, not asserted.** If `_shared/slop-engine/` is absent, `detect.mjs` throws, or the vendored parser can't process the HTML, the runner itself prints the non-fatal skip-note above and writes a `skipped:true` report at exit `0`; `/verify` continues with prior behaviour. You may record "skipped / engine unavailable" **only** when the runner actually emitted that skip-note (its `skipped:true` JSON) — never by inferring tooling absence yourself. This mirrors the `#hard-gates` comments-coverage existence-guard and the browser-tool-ladder rung-4 logged-skip below. A check the Node parser can't reproduce (the engine's layout-dependent `BROWSER_ONLY_RULES`) is skipped on the Node path with a logged note — it runs via `/design-crit`'s browser path (story B) — never silently dropped.
...
- Do NOT claim the Slop gate (`#slop-gate`) degraded — that the slop engine/parser was unavailable (the Inv-5 skip) — without the runner's own skip-note (`[slop-gate] slop gate skipped — engine/parser unavailable: …` and a `skipped:true` JSON report). Graceful degradation is a determination earned from the runner's emitted line + exit `0`, never asserted from narrative or inferred tooling absence. (This is distinct from the legitimate trigger-negative `slop gate: skipped — no UI surface` log, which the Phase 4 entry gate decides without running the runner.) Likewise, do NOT record a gate fire or pass without surfacing the runner's verbatim run-summary line for this run.
