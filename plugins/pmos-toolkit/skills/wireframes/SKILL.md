...
- **The nearest worked example from `reference/examples.md`, with the instruction: START FROM IT AND MODIFY — do not build the payload from blank** (the reference skill's own rule; from-blank fan-out is the drift the `#review` Rigor protocol catches). Pick the nearest by shape: a desktop multi-region page → example 1 (dashboard); a single mobile form → example 2; an overlay/dialog → example 3; **any screen with more than one required state (empty/loading/error) → example 4 (multi-state)**. Pass that one example's complete payload into the subagent's payload alongside the template.
...
**Generate `{feature_folder}/wireframes/index.html`** — a launcher + narrative surface. Its base layer is a card grid linking every `(component × device)` file (name, device chip, state-count badge, small preview), filterable by device and by name, working offline from `file://`, using Tailwind CDN + the shared `./assets/wireframe.css`. Header links back to the req doc and to `canvas.html` ("Canvas view (all devices)") — rendered unconditionally; `#canvas` always emits the file moments later. With a single target device, omit the device-filter row (a one-tab control is noise) and note the omission in the footer.

**Per-screen narrative row (additive — amendment A7: fold into `index.html`, do not add a third surface).** Beneath the launcher grid, emit one narrative row per screen so `index.html` tells the story of *what changed and why*, not just *where to click*. Each row lays out, left-to-right:

1. **The wireframe** — the same inline preview / link to the per-screen file the launcher card already carries (reuse it; do not re-render the SVG a second way).
2. **The ingested source screenshot** — the matching image from `{feature_folder}/wireframes/assets/source-screens/` (the ingest target populated by `--screenshots`, per `reference/screenshot-ingestion.md`), shown as an `<img>` with a **relative** `src` so it resolves from `file://` with no server. Match the screen to its screenshot via the **Anchored to** line in `assets/source-screens.md`.
3. **The numbered annotation list** — read **from that screen's `<script type="application/json" id="pmos-wireframe-meta">` manifest** (the `annotations` array of `{n, note, anchor, state}` written by `#generate` per `reference/html-template.md`). Render the list keyed to the `<!-- N: … -->` primitive markers. **[§K — one fact, one home]** the list is READ from the manifest, **never re-derived or hand-authored**, so the narrative and the wireframe cannot drift. Because the manifest is inline JSON in the per-screen file, inline the same objects into `index.html` at generate time (or parse the sibling file) — no `fetch` (it would fail under `file://`).

**Optional-screenshot rule (screenshot ingestion is optional and stays optional).** A screen with **no** matching file in `assets/source-screens/` renders its narrative row with the **screenshot column absent** — collapse the row to *wireframe + annotation list* only. Do **not** emit a broken `<img>`, an empty placeholder box, or any "a screenshot was expected here" affordance. Make the emit **conditional on per-screen file existence, not on a global flag**: a mixed run (some screens ingested, some not) renders each row correctly, and a run invoked with no `--screenshots` produces an index with zero screenshot columns that reads identically clean. The numbered annotation list from the manifest still renders for every screen regardless of screenshot presence.

**Preserve the launcher (amendment A5 — no de-Tailwind).** The narrative is layered *onto* the existing surface: keep every per-screen file link and the `canvas.html` ("Canvas view (all devices)") link, keep the Tailwind CDN load and `./assets/wireframe.css` (this story does not de-Tailwind `index.html`; add no prose claiming it did), and keep the inline `pmos-comments` block + comments overlay assets exactly as before (`check-comments-coverage.sh` must still pass). The page stays readable from `file://` with no server.
