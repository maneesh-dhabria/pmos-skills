...
// structure must return agent_judged_infeasible.
  ...
  return Object.assign(
    {
      ...
      thread_id: "T_A",
      anchor: { id_anchor: "header-bar", quote_anchor: null },
      body: "tighten the breadcrumb description",
    },
    overrides || {}
  );
}
...
(async () => {
    ...
    const out = await apply(
      mkInput({
        ...
        anchor: {
          ...
          quote_anchor: { text: "a phrase that does not appear anywhere in the wireframe at all xyzzy plugh" },
        },
        body: "fix this label",
      })
    );
  ...
  // (d) agent_judged_infeasible: cross-section restructure heuristic.
  try {
    const out = await apply(
      mkInput({
        thread_id: "T_D",
        anchor: { id_anchor: "header-bar" },
