/**
 * Anti-Pattern Browser Detector for pmos slop-engine
...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__PMOS_SLOP_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip pmos-slop's own elements
    if (el.closest && el.closest('.pmos-slop-overlay, .pmos-slop-label, .pmos-slop-banner, .pmos-slop-tooltip')) continue;
...
if (IS_BROWSER) {
  ...
  styleEl.textContent = `
    ...
    .pmos-slop-overlay:not(.pmos-slop-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: pmos-slop-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .pmos-slop-overlay.pmos-slop-visible {
      animation-play-state: running;
    }
    .pmos-slop-overlay.pmos-slop-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .pmos-slop-overlay.pmos-slop-hover .pmos-slop-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .pmos-slop-overlay.pmos-slop-spotlight {
      z-index: 100002 !important;
    }
    .pmos-slop-overlay.pmos-slop-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .pmos-slop-spotlight-backdrop {
      position: fixed;
    ...
    .pmos-slop-spotlight-backdrop.pmos-slop-visible {
      opacity: 1;
    }
    .pmos-slop-hidden .pmos-slop-overlay${EXTENSION_MODE ? '' : ':not(.pmos-slop-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'pmos-slop-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__PMOS_SLOP_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('pmos-slop-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('pmos-slop-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('pmos-slop-banner')) continue;
      ...
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('pmos-slop-visible');
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('pmos-slop-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    ...
    outline.className = 'pmos-slop-overlay';
    ...
    label.className = 'pmos-slop-label';
    ...
    const onMouseEnter = () => {
      isHovered = true;
      outline.classList.add('pmos-slop-hover');
    ...
    const onMouseLeave = () => {
      isHovered = false;
      outline.classList.remove('pmos-slop-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'pmos-slop-overlay pmos-slop-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('pmos-slop-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('pmos-slop-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  function collectVisualContrastCandidates(options = {}) {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      if (el.closest('.pmos-slop-overlay, .pmos-slop-label, .pmos-slop-banner, .pmos-slop-tooltip')) continue;
      if (el.closest('[id^="pmos-slop-live-"]')) continue;
  ...
  async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
    ...
    for (const node of nodes) {
      ...
      if (node.closest?.('.pmos-slop-overlay, .pmos-slop-label, .pmos-slop-banner, .pmos-slop-tooltip')) continue;
  ...
  function serializeFindings(allFindings) {
    return allFindings.map(({ el, findings }) => ({
      ...
      findings: findings.map(f => {
        const ap = SLOP_RULES.find(a => a.id === (f.type || f.id));
        return {
          type: f.type || f.id,
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      console.log('%c[pmos-slop] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
      `%c[pmos-slop] ${allFindings.length} anti-pattern${allFindings.length === 1 ? '' : 's'} found`,
  ...
  function browserDesignSystemConfig() {
    const raw = window.__PMOS_SLOP_CONFIG__?.designSystem;
  ...
  function collectBrowserFindings() {
    ...
    const _disabled = EXTENSION_MODE ? (window.__PMOS_SLOP_CONFIG__?.disabledRules || []) : [];
    ...
    for (const el of document.querySelectorAll('*')) {
      // Skip pmos-slop's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.pmos-slop-overlay, .pmos-slop-label, .pmos-slop-banner, .pmos-slop-tooltip')) continue;
      // Skip browser extension elements (Claude, etc.)
      const elId = el.id || '';
      if (elId.startsWith('claude-') || elId.startsWith('cic-')) continue;
      // Skip the pmos-slop live-mode overlay (highlight, tooltip, bar, picker, toast).
  ...
  function shouldRunVisualContrast(options = {}) {
    return options.visualContrast === true || window.__PMOS_SLOP_CONFIG__?.visualContrast === true;
  }

  function visualContrastOptions(options = {}) {
    const config = window.__PMOS_SLOP_CONFIG__ || {};
  ...
  function postExtensionError(err) {
    if (!EXTENSION_MODE) return;
    window.postMessage({
      source: 'pmos-slop-error',
      message: err?.message || String(err),
    }, '*');
  }

  function reportVisualContrastError(err, detail = {}) {
    window.dispatchEvent(new CustomEvent('pmos-slop-visual-contrast-error', {
      detail: {
        ...detail,
        message: err?.message || String(err),
      },
    ...
    if (EXTENSION_MODE) {
      ...
      console.warn('[pmos-slop] visual contrast scan failed', err);
    }
  }
        ...
        waitForVisualPaint()
          ...
          .then(result => {
            ...
            if (added) {
              ...
              window.dispatchEvent(new CustomEvent('pmos-slop-visual-contrast-resolved', {
                detail: {
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      if (e.source !== window || !e.data || e.data.source !== 'pmos-slop-command') return;
      if (e.data.action === 'scan') {
        if (e.data.config) window.__PMOS_SLOP_CONFIG__ = e.data.config;
        try {
          scan(e.data.config || {});
        } catch (err) {
          postExtensionError(err);
      ...
      if (e.data.action === 'toggle-overlays') {
        const visible = !document.body.classList.contains('pmos-slop-hidden');
        document.body.classList.toggle('pmos-slop-hidden', visible);
        window.postMessage({ source: 'pmos-slop-overlays-toggled', visible: !visible }, '*');
      }
      if (e.data.action === 'remove') {
        clearOverlays();
        styleEl.remove();
        if (spotlightBackdrop) { spotlightBackdrop.remove(); spotlightBackdrop = null; }
        document.body.classList.remove('pmos-slop-hidden');
      }
      if (e.data.action === 'highlight') {
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('pmos-slop-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('pmos-slop-spotlight', isMatch);
              o.classList.toggle('pmos-slop-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('pmos-slop-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('pmos-slop-spotlight');
          o.classList.remove('pmos-slop-spotlight-dimmed');
        }
      }
    });
    window.postMessage({ source: 'pmos-slop-ready' }, '*');
  } else {
    if (window.__PMOS_SLOP_CONFIG__?.autoScan !== false) {
      const runAutoScan = () => {
        try {
          scan();
        } catch (err) {
          console.warn('[pmos-slop] scan failed', err);
        }
      };
