...
function getAntipattern(id) {
  return SLOP_RULES.find(rule => rule.id === id);
}

function getRulesForCategory(category) {
  return SLOP_RULES.filter(rule => rule.category === category);
}
...
const GATED_PROVIDERS = new Set(
  SLOP_RULES.map(rule => rule.gated).filter(Boolean),
);
