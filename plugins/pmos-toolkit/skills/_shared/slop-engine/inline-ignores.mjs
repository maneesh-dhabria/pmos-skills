 ...
 * an ignore in `.pmos-slop/config.json`.
 *
 ...
 * line, so the same marker works across every comment style pmos-slop scans —
 * `//`, `/* *\/`, `<!-- -->`, `#`, `{/* *\/}`, `{# #}`. Trailing comment closers
 * are stripped before the rule list is parsed.
 *
 * Syntax (reason optional; eslint `--` or biome `:` separator):
 *
 *   pmos-slop-disable <rule>[, <rule>...] [-- reason]   whole file
 *   pmos-slop-disable-line <rule>...      [-- reason]   the same line
 *   pmos-slop-disable-next-line <rule>... [-- reason]   the following line
 *   pmos-slop-disable                                   bare / `*` = every rule
 *
 * Examples:
 *
 *   <!-- pmos-slop-disable overused-font -- exported brand doc, font is first-party -->
 *   .brand { font-family: Inter; } /* pmos-slop-disable-line overused-font *\/
 *   // pmos-slop-disable-next-line bounce-easing: intentional playful affordance
 *
...
const DIRECTIVE_RE = /pmos-slop-(disable-next-line|disable-line|disable)\b[ \t]*([^\n\r]*)/gi;
...
function parseInlineIgnores(content) {
  ...
  // Case-insensitive to match DIRECTIVE_RE's `i` flag (e.g. `pmos slop-engine-Disable`).
  if (!/pmos-slop-disable/i.test(text)) return result;
