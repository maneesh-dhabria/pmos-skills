...
import { GENERIC_FONTS, OVERUSED_FONTS } from './constants.mjs';
import {
  checkSourceDesignSystem,
  collectStaticDesignSystemFindings,
  mergeDesignSystemFindings,
} from './design-system.mjs';
import { isFullPage } from './page.mjs';
import { applyInlineIgnores } from './inline-ignores.mjs';
import { finding } from './findings.mjs';
import { profileFindings, profileStep, profileStepAsync } from './profiler.mjs';
import {
  checkElementBorders,
...
import { filterByProviders } from './registry.mjs';
import { detectText, runTextContentAnalyzers } from './detect-text.mjs';
import {
  StaticDocument,
  buildStaticStyleMap,
  buildStaticWindow,
...
// Inv-4/Inv-5: the no-layout Node path resolves computed style structurally (parseFloat on
// declared px), but checks that need REAL rendered geometry — getBoundingClientRect, viewport
// position, hover state — cannot be fully evaluated here. They are NOT silently dropped: they
// degrade to the browser path (browser.js / /design-crit), and detectHtml emits a one-time note
// listing them so a consumer (e.g. /verify, story C) knows the Node run is a partial pass.
const BROWSER_ONLY_RULES = [
  'icon-tile-stack',            // vertical stacking above a heading needs layout box positions
  'body-text-viewport-edge',    // proximity to the viewport edge is a rendered-geometry test
  'image-hover-transform',      // hover/transform state only exists in a live browser
  'oversized-h1',               // viewport-relative sizing depends on the rendered viewport
];
let _degradedNoticeEmitted = false;

async function detectHtml(filePath, options = {}) {
  const profile = options?.profile;
  if (!_degradedNoticeEmitted && options?.logDegraded !== false) {
    _degradedNoticeEmitted = true;
    console.error(
      `[slop-engine] Node static path: ${BROWSER_ONLY_RULES.length} layout-dependent rules degrade `
      + `to browser-only (${BROWSER_ONLY_RULES.join(', ')}); run the browser detector for full coverage.`,
    );
  }
  // AC9: accept either a file path OR a raw HTML string. An argument that looks like
  // markup is treated as inline HTML; external CSS @import/<link> then resolve from cwd.
  const isInlineHtml = typeof filePath === 'string'
    && filePath.includes('>')
    && /<(?:!doctype|html|head|body|main|section|article|div|span|h[1-6]|p|a|ul|table|style|[a-z][\w-]*[\s/>])/i.test(filePath);
  let html;
  if (isInlineHtml) {
    html = filePath;
    filePath = options?.sourceLabel || 'inline.html';
  } else {
    html = profileStep(profile, {
      engine: 'static-html',
      phase: 'setup',
      ruleId: 'read-html',
      target: filePath,
  ...
  try {
    modules = await profileStepAsync(profile, {
      ...
      const parsers = await import('./vendor/parsers.mjs');
      return {
        parseDocument: parsers.parseDocument,
        selectAll: parsers.selectAll,
        selectOne: parsers.selectOne,
        is: parsers.is,
        csstree: parsers.csstree,
        domutils: parsers.domutils,
      };
    });
  } catch {
  ...
  // `pmos-slop-disable` directives apply here — exactly the standalone-document
...
export { checkStaticPageTypography, STATIC_ELEMENT_RULES, BROWSER_ONLY_RULES, detectHtml };
