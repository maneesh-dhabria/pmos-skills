#!/usr/bin/env node
// /wayrttd "Apply comment-resolver edit" Node-callable shim.
//
...
// ≥40-char quote grounding). Surface area kept ≤ ~200 LOC.
//
// Skill-specific feasibility: no special read-only regions for /wayrttd — all
// five capture sections (solution-x, intent-ladder, problem-y, alternatives,
// verdict) are editable via standard anchor resolution.
...
// Quote must be ≥40 chars to avoid false matches.
...
async function apply(input) {
  try {
    ...
    // 5. Propose edit + synthesize applied_artifact. The shim does NOT do real
    //    prose rewriting (that is the originating skill's deeper machinery). It
    //    inserts a single HTML annotation comment immediately before the
    //    resolved anchor element capturing the operator's request — byte-real
    //    (shows in git diff), idempotent via the per-process ledger.
    _applied.add(key);
    const r = resolved.dom_range;
    const bodyExcerpt = String(input.body || "").slice(0, 80).replace(/-->/g, "--&gt;");
    const annotation = `<!-- comment-resolver: thread=${input.thread_id} skill=wayrttd request="${bodyExcerpt}" -->\n`;
