...
- **Missing substrate assets (two tiers):** the `cp -n` sources in Phases 3.6 and 7 are classified hard-required (`survey-preview.js`, `style.css`, `viewer.js`) vs. convenience (`serve.js`). A missing **hard-required** source ⇒ abort with `survey-design: missing substrate asset <abs-path> — reinstall the pmos-toolkit plugin` and write no degraded artifact; a missing **convenience** source ⇒ one warning, skip the dependent extra, continue. Never improvise a self-contained renderer in place of the substrate. (The MD-sidecar emit path was retired in FR-12.1.)
...
**Theater-check escape (FR-SR-5 pattern — see `readme/reference/simulated-reader.md` §3).** A persona that returns **empty** `friction`/`confusion`/`comprehension_gaps` across **every** question *and* no `dropoff_risk_points`, while the Phase-4 reviewer (`question-eval.md`) produced **≥3 findings**, is suspect — it's likely "I'm being a cooperative respondent and nothing's wrong" theater. Re-dispatch that one persona **ONCE** with this suffix appended to the prompt body — spec the persona's *constraints + bounce conditions*, not a biography:
...
Accept the second result as genuine even if it is still empty. **No second retry** (1-iteration cap — no convergence loop). Log: `survey-design: <persona> re-dispatched (theater-check); <N> findings on retry`.
...
| E21 | A convenience substrate asset (`serve.js`) is missing at copy time | One warning `survey-design: convenience asset <path> missing — skipping <dependent feature>`; skip the dependent extra (preview server); continue — `survey.html` / `preview.html` still produced (Phases 3.6, 7). |
