#!/usr/bin/env node
// T18 — per-skill contract test for /requirements's "Apply comment-resolver edit" shim.
...
const FIXTURE = path.join(__dirname, "fixtures", "apply-edit-at-anchor", "requirements_mini.html");
...
(async () => {
  ...
  try {
    const out = await apply(
      mkInput({
        thread_id: "T_E",
        anchor: { id_anchor: "open-questions" },
    ...
    assert.deepStrictEqual(
      ...
      ["eager", "lazy"],
      "(e) options split on ' or ' and trimmed"
    );
  } catch (e) {
  ...
  if (failures.length > 0) {
    console.error("FAIL: /requirements apply-edit-at-anchor — " + failures.length + " of 5 cases");
    for (const f of failures) console.error("  - " + f);
    process.exit(1);
  }
  console.log("PASS: /requirements apply-edit-at-anchor — 5 cases");
})().catch((e) => {
