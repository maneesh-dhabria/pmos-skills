#!/usr/bin/env node
// T18 — /requirements "Apply comment-resolver edit" Node-callable shim.
//
...
// /requirements's per-skill apply-edit phase. Surface area kept ≤ ~200 LOC.
...
// Heuristic: out-of-scope restructure of prose requirements sections.
...
function _maybeClarification(body) {
  ...
  // Find the " X or Y" tail. Strip leading scaffolding.
...
async function apply(input) {
  try {
    ...
    // 5. Propose edit + synthesize applied_artifact. The shim inserts a
    //    single HTML annotation comment immediately before the resolved
    //    anchor element capturing the operator's request — byte-real (shows
    //    in git diff), idempotent via the per-process ledger.
    _applied.add(key);
    const r = resolved.dom_range;
    const bodyExcerpt = String(input.body || "").slice(0, 80).replace(/-->/g, "--&gt;");
    const annotation = `<!-- comment-resolver: thread=${input.thread_id} skill=requirements request="${bodyExcerpt}" -->\n`;
