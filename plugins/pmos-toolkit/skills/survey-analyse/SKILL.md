...
- **Missing substrate assets:** the asset payload under `_shared/html-authoring/assets/` is the hard-required substrate for Phase 7 — if any of `style.css`, `viewer.js` is missing, abort with `survey-analyse: missing substrate asset <abs-path> — reinstall the pmos-toolkit plugin`.
...
This skill runs 9 sequential phases (0–8) — one task per phase below. Create them via `TaskCreate`; mark each in-progress on start and completed on finish — don't batch. The run folder's `index.html` mirrors the task state for the user.
...
2. **Parse flags** from the argument string: `--responses <path>` (required), `--survey-json <path>`, `--sheet <name|N>`, `--raw-p-only`, `--skip-cleaning`, `--non-interactive | --interactive`. NL-first: a bare path in the request ("analyse responses.csv") ≡ `--responses`; an explicit flag overrides. Missing responses path either way → stderr usage line; exit 64.
...
## Phase 1 — Ingest responses {#ingest}
...
## Phase 2 — Schema + context intake (always user-confirmed) {#schema}

1. **If `--survey-json` was passed:** read it; treat `sections[].questions[]` as the proposed schema (column = question; type = the question's `type` field; the IR's shape is documented in `/survey-design`'s `reference/survey-json-schema.md` — don't assume fields it doesn't define). Match columns by question wording (fuzzy) or question id.
...
## Phase 4 — Per-question analysis (LLM-authored analysis.py + Bash run) {#analysis}
...
3. **Self-check before running:** `analysis.py` contains no statistical computation of its own — every per-question number in `per_question.json` is a helper return value verbatim (Anti-Pattern #1; a quick scan for arithmetic over response values in the authored script is enough).
4. **Run it once via Bash** (`python3 analysis.py`) — a single consolidated permission ask covers the run. If the user denies: stop with the script saved; user can run it themselves and paste output back (Platform-Adaptation fallback).
5. **Helper output flows verbatim into the report.** The LLM narrates on top — it does NOT recompute frequencies, T2B, NPS, etc. from the CSV.

## Phase 5 — Open-end thematic coding (subagent per question) {#thematic-coding}

For each column with `type: open_text` and `n ≥ 5` non-null responses:

1. **Dispatch a fresh subagent** with `model: sonnet` (bounded thematic coding against the Braun & Clarke contract; step 2 validates every returned `response_id` parent-side — the cheaper tier is safe) and the verbatim prompt template from `reference/text-analysis.md` — includes the question wording, the verbatims (chunked at ≤200 per call to stay within subagent context; multi-call concatenation if needed), and the Braun & Clarke 6-phase contract.
...
## Phase 6 — Whole-survey synthesis (cross-tabs + Holm) {#synthesis}
...
## Phase 7 — Report rendering {#report}
...
1. **Sections + content** per the "Report skeleton" + executive-summary / "so what" / chart-hygiene / honest-reporting rules in `reference/data-quality-and-reporting.md` Part B (loaded in Phase 3) — seven sections in its order, each a `<section id="…">`; chart choice per question type per `reference/question-type-analysis.md`. Two emphases the skeleton compresses: the methodology section's weighting status is always "unweighted" (`reference/cross-survey-stats.md` §Weighting), and the **PII warning summary** from Phase 5 goes in methodology too.
...
5. **`output_format: both`** → retired with the inline-html-artifacts cutover; treated as `html` until a future feature re-introduces MD export.

## Phase 8 — Handoff + capture learnings {#handoff}
...
- **Capture learnings.** Reflect on whether this session surfaced anything worth capturing under `## /survey-analyse` in `~/.pmos/learnings.md` — a repeated correction, a schema-inference miss, a question-type the helpers handled poorly, a PII pattern the regex didn't catch. Emit exactly one line: either the new learning appended under that header, or `No new learnings this session because <specific reason>`. Proposing zero learnings is a valid outcome.

## Apply comment-resolver edit {#apply-comment-resolver-edit}

The `/survey-analyse` entrypoint that `/comments resolve` dispatches into when walking open threads in a report artifact's inline `pmos-comments` JSON block.
...
- **Skill-specific feasibility:** edits to chart data (any `<script type="application/json">` block or chart-config block) return `agent_judged_infeasible` with `system_reply`: `"Chart data is generated from the response set — re-run /survey-analyse with updated responses."` Detection: anchor `id_anchor` matching `chart-*`, `pmos-chart-*`, `data-block-*`, or `chart-config-*`; or `quote_anchor.text` containing chart JSON keys (`"labels"`, `"datasets"`, `"data"`, `"type"`); or post-read detection that the anchor position falls inside a `<script type="application/json">` block. Analysis prose (executive-summary, key-findings, methodology, open-end-themes, etc.) IS editable.
- **Tests:** `tests/apply-edit-at-anchor.test.js` (5 cases) + wrapper `tests/scripts/assert_apply_edit_at_anchor_survey-analyse.sh`.

---

## Anti-Patterns (DO NOT)

1. **Do NOT have the LLM compute summary statistics by inspecting the CSV.** Numbers come from the helper modules via `analysis.py`. The LLM narrates on top. (The whole reproducibility contract rides on this.)
...
7. **Do NOT auto-redact PII in verbatim quotes.** Detect-and-warn only. False-positive redactions ("Apple" → `[name]`) hurt signal more than they help; user takes responsibility for scrubbing before external sharing. (User-confirmed design choice.)
...
*Spec lineage: `docs/pmos/features/2026-05-13_survey-analyse/` (pipeline, helper layer, reproducibility + PII contracts), `2026-05-23_inline-doc-comments/` + `2026-05-28_inline-html-artifacts/` (comment resolver, inline persistence), `2026-05-08_non-interactive-mode/` (inline mode block). Per-rule FR traceability lives in those specs, not inline here.*
