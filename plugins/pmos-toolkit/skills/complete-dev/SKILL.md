...
Per-epic destructive DEFER gates (PASS-WITH-GAPS, the `--stories` subset confirm) are **preserved verbatim** per epic (D8/AC7); under `--non-interactive` they DEFER→abort that epic's train — which the stop-and-report rule above then treats as a failed train (no separate loop-halt logic). The W14 inline non-interactive block is untouched (I4).
...
2. **Integrated deterministic gate (D14 — new, runs on the merged tree BEFORE any version bump).** Two independently-PASSing stories can still interact badly. Run the repo's deterministic suite on the merged main — the test command(s), lints, and any hard-gate scripts detected in Phase 5's deploy-norms probe (e.g. `npm test`, the repo's `tools/*.sh` hygiene lints, skill-eval where applicable). **Red ⇒ the train stops: `/backlog set <epic-id> status=blocked`, nothing is bumped or tagged, surface the failing output, exit.** This is a deterministic gate only — no fresh LLM review pass (the 0-of-27 judge-rerun yield, per `06_self-verification-value.md`).
   ...
   **`route: skill` epics — additionally run the `[D]` skill-eval pass AND one coherence `[J]` pass (G6).** For an epic whose stories are `route: skill`:
   - **`[D]` half on the merged tree** — run `feature-sdlc/tools/skill-eval-check.sh --target <platform> <skill_dir>` for every changed skill dir; any fail is a red gate (same stop semantics). This is the concrete realization of "skill-eval where applicable" above.
   - **One epic-level coherence `[J]` pass** — dispatch a single reviewer subagent (temperature 0) scoped to *cross-skill coherence only*: do the merged skills still agree on the shared-substrate contracts, do aliases and the skills they forward to stay consistent, do the `design_doc:` invariants still hold? This is **not** a per-skill `skill-eval` rerun (that ran per-story in `build`, and D14's 0/27 finding makes reruns worthless) — it is a new check that only the assembled batch can surface, and the sole judgement gate before an unattended-built epic ships. Findings ⇒ red gate: train stops, `/backlog set <epic-id> status=blocked`, the coherence findings appended to the epic body so they surface in `/backlog groom`. Green ⇒ proceed. (`route: feature|lite` epics skip this sub-step — it is skill-coherence-specific.)
3. **One changelog (replaces Phase 8).** Assemble a single `/changelog` entry for the whole epic from the shipped stories' one-line summaries (each story's `/verify` verdict line + title), not per-story entries. **`route: skill` epics — split user-facing vs maintenance (G5):** each story self-classifies `user-facing | maintenance` (the `/verify` verdict already summarizes the change; classify from it). Render **user-facing** stories as capability entries ("what the system can now do"); **roll all maintenance** stories (refactors, policy codification, eval-rubric tightening) into a single `Skill quality & internals` line. This honours `/changelog`'s "what it can do, not how it was built" contract — a pure-internal skill epic produces one maintenance line, not N awkward how-it-was-built entries.
...
6. **Worktree cleanup (replaces Phase 16a).** Remove **all** story worktrees of the epic (each story's `worktree:`), not just one feature branch — same dirty-check + `ExitWorktree` + branch-delete discipline as Phase 16a, looped over the story set. Clear each story's `worktree:` field. **This loop owns per-*story* worktrees only — the epic's `define/<epic-id>` worktree is owned and torn down by `/feature-sdlc define` (`feature-sdlc/SKILL.md#define-mode` step 5) at the definition merge, never here (invariant I3).**
