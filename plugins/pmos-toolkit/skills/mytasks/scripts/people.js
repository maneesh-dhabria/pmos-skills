...
function createPerson(peopleDir, input) {
  ...
  regenPeopleIndex(peopleDir, today);
...
function patchPerson(peopleDir, handle, fields) {
  ...
  regenPeopleIndex(peopleDir, fm.updated);
  return { record: recordJson(fm) };
}
...
// ── INDEX.md regeneration (people/schema.md "INDEX.md format") ──
function regenPeopleIndex(peopleDir, today) {
  const people = [];
  let names;
  try { names = fs.readdirSync(peopleDir); } catch (_) { names = []; }
  for (const n of names) {
    if (!n.endsWith('.md') || n === 'INDEX.md') continue;
    try { people.push(parsePerson(fs.readFileSync(path.join(peopleDir, n), 'utf8')).fm); } catch (_) { /* skip */ }
  }
  people.sort((a, b) => String(a.name || '').localeCompare(String(b.name || '')) || String(a.handle || '').localeCompare(String(b.handle || '')));
  const cell = (v) => (v === undefined || v === null) ? '' : String(v);
  let md = `# People\n\nLast regenerated: ${today || lib.isoToday()}\n\n`;
  md += `| handle | name | designation | role | working_relationship | team | email |\n`;
  md += `|--------|------|-------------|------|----------------------|------|-------|\n`;
  for (const fm of people) {
    md += `| ${cell(fm.handle)} | ${cell(fm.name)} | ${cell(fm.designation)} | ${cell(fm.role)} | ${cell(fm.working_relationship)} | ${cell(fm.team)} | ${cell(fm.email)} |\n`;
  }
  fs.mkdirSync(peopleDir, { recursive: true });
  lib.writeItemAtomic(path.join(peopleDir, 'INDEX.md'), md);
  return md;
}
...
module.exports = {
  ...
  listPeople, readPerson, createPerson, patchPerson, regenPeopleIndex, recordJson,
};
