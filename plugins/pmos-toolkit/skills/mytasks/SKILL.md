...
user-invocable: true
argument-hint: "[ | <text> | add <text> [--parent <id>] | list [filters] | today | week | overdue | waiting | checkins | for <handle> | in <project> | show <id> | set <id> <field>=<value> | refine <id> | done <id> [note] | drop <id> [reason] | checkin <id> [note] | archive [--quarter <YYYY-QN>] | rebuild-index | web | import [<text>]] [--non-interactive | --interactive]"
---
...
5. **Regenerate `INDEX.md` once** at the very end (`#rebuild-index`), not per task — same fail-soft semantics as `#quick-capture` (a write that fails INDEX regen still leaves the item files; warn, do not roll back).
