#!/usr/bin/env node
// T19 — /prototype "Apply comment-resolver edit" Node-callable shim.
//
...
//
// /prototype emits a single per-device HTML file with embedded React via
// CDN + simulated API. Applyable surface: textual/HTML edits inside
// <section> regions. Infeasible surface: edits inside <script> JSX blocks
// or the simulated mock-data block — those require regeneration via
// /prototype.
//
// Minimal in-shim anchor resolver (id-first + Bitap-via-dmp fallback).
// Surface area kept ≤ ~200 LOC.
...
// Infeasibility: edits inside <script> JSX blocks or mock-data blocks
// require prototype regeneration. Heuristic: body mentions JSX / React
// component internals or mock data restructuring.
...
async function apply(input) {
  try {
    ...
    // 1. Infeasibility heuristic (prototype-specific: JSX / mock-data).
    if (_isInfeasible(input.body)) {
      return {
        success: false,
        error_enum: ERROR_ENUM.AGENT_JUDGED_INFEASIBLE,
        system_reply:
          "Cannot apply: edit targets JSX script block or simulated mock-data. Regenerate the prototype via /prototype to apply structural React or data changes.",
      };
    }
    ...
    const annotation = `<!-- comment-resolver: thread=${input.thread_id} skill=prototype request="${bodyExcerpt}" -->\n`;
