...
1. **Extract visible-field summary** from wireframes. **Primary source — the screen manifest:** for each `wireframes/*.html`, read + `JSON.parse` its inline `<script type="application/json" id="pmos-wireframe-meta">` block and take the declared `fields` array (emitted per screen by monochrome-SVG `/wireframes`). This is the declared replacement for the tag-grep — SVG payloads render form fields as `<text>` nodes, so `<th>`/`<label>`/`<dt>`/`data-field` do not exist in them. **Legacy fallback (pre-refactor wireframes only):** a screen with no `pmos-wireframe-meta` block falls back to grepping `<th>`, `<label>`, `<dt>` text and `data-field` attributes. Build `{screen, fieldsShown: [...], approxRowCount: N}`. **Silent-empty guard:** if, for a screen, neither the manifest nor the legacy tag-grep yields any fields, do NOT proceed with an empty model — report the screen by name (and that its entity model would be empty) to the user and pause; a zero-match that proceeds silently is the dangerous failure this replaces.
...
1. **Extract visible-field summary per screen** — primary source is each screen's `<script type="application/json" id="pmos-wireframe-meta">` manifest (`fields` + `components`, per Phase 4 step 1); the `<th>`/`<label>`/`<dt>`/button/CTA tag-grep survives only as the legacy fallback for pre-refactor wireframes with no manifest → `{feature_folder}/prototype/.screens-summary.json` for the generator subagents. The same silent-empty guard applies: a screen that yields no fields from either source is reported, not written as an empty summary.
