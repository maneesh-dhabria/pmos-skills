...
The `## Epics` rollup table lists every epic with a **derived** `stories (done/total)` count (non-wontfix stories) — collapses single-story epics into one row. Stories are then grouped by `priority` (must > should > could > maybe), sorted within each group by `score` desc (nulls last), then `updated` desc. Archived items are NOT listed. The `spec` / `plan` / `pr` columns show the filename only (not the full path) when set, otherwise blank.
