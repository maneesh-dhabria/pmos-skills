#!/usr/bin/env node
// /shape "Apply comment-resolver edit" Node-callable shim.
//
...
//
// Skill-specific feasibility: no special read-only regions for /shape —
// all prose sections are editable via standard anchor resolution. A comment
// that asks to add a *solution* to a problem-only brief is out of charter and
// is left open for the user (handled by the resolver, not this shim).
...
async function apply(input) {
  try {
    ...
    // 5. Propose edit + synthesize applied_artifact. The shim inserts a single
    //    HTML annotation comment immediately before the resolved anchor element
    //    capturing the operator's request — byte-real (shows in git diff),
    //    idempotent via the per-process ledger.
    _applied.add(key);
    const r = resolved.dom_range;
    const bodyExcerpt = String(input.body || "").slice(0, 80).replace(/-->/g, "--&gt;");
    const annotation = `<!-- comment-resolver: thread=${input.thread_id} skill=shape request="${bodyExcerpt}" -->\n`;
