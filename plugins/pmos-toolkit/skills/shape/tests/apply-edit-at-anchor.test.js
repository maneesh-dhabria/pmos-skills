#!/usr/bin/env node
// Per-skill contract test for /shape's "Apply comment-resolver edit" shim.
...
const FIXTURE = path.join(__dirname, "fixtures", "apply-edit-at-anchor", "shape_mini.html");
  ...
  return Object.assign(
    {
      ...
      thread_id: "T_A",
      anchor: { id_anchor: "tldr", quote_anchor: null },
...
(async () => {
  ...
  try {
    const out = await apply(
      mkInput({
        thread_id: "T_E",
        anchor: { id_anchor: "felt-problem" },
        body: "should this be framed as retention or activation?",
      })
    );
  ...
  if (failures.length > 0) {
    console.error("FAIL: /shape apply-edit-at-anchor — " + failures.length + " of 5 cases");
    for (const f of failures) console.error("  - " + f);
    process.exit(1);
  }
  console.log("PASS: /shape apply-edit-at-anchor — 5 cases");
})().catch((e) => {
