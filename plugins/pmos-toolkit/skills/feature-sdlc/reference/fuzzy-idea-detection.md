# Fuzzy-idea detection (Phase 1.5)

Deterministic heuristic used by `/feature-sdlc` Phase 1.5 to classify the run's seed as **fuzzy** (warrants `/ideate`) or **formed** (skip straight to `/requirements`). Mirrors the shape of `frontend-detection.md` — small, transparent, side-effect-free.
