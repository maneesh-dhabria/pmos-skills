...
| Name | Type | Source |
|---|---|---|
| `seed_text` | string | The user's argument string to `/feature-sdlc` (or to `/feature-sdlc skill`), with recognised flags removed (`--tier`, `--resume`, `--no-worktree`, `--format`, `--non-interactive`, `--interactive`, `--backlog`, `--minimal`, `--no-ideate`, `--reset-defaults`, `--from-feedback`, `--from-reflect`) and surrounding quotes stripped. |
