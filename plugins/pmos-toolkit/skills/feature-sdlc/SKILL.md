...
- **`skill --from-feedback <source>` (FR-04):** `pipeline_mode = skill-feedback`. `<source>` is a quoted text blob, a file path, or `--from-reflect`. `--from-reflect` resolves to the `/reflect` output present in the current conversation (the in-session report — `/reflect` by design writes no artifact file); if the conversation contains no `/reflect` output → stderr `no /reflect output in this conversation; pass feedback text or a file path`, exit 64. `skill --from-feedback` with neither a source nor `--from-reflect` → the FR-03 usage error, exit 64.
...
**In skill modes** (FR-61 — fuller wiring in the citation pass): prepend a line citing `reference/skill-patterns.md` as the standing acceptance criteria — "the produced/revised skill must conform to `skill-patterns.md §A–§L`".
...
> **Where the conventions live:** `reference/skill-patterns.md` carries the *generic, repo-agnostic* skill-authoring conventions (frontmatter, description triggering, progressive disclosure, body content, scripts, platform-conditional frontmatter, release scope, gates, flags, phases, one-fact-one-home, subagent/model selection — §A–§L). This repo's **`CLAUDE.md ## Skill-authoring conventions`** carries the *pmos-specific* bits: the canonical `plugins/pmos-toolkit/skills/<name>/SKILL.md` path, the synced `plugin.json` version bump, and `/complete-dev` as the release entry point (FR-62). `/execute` (and the Phase 6a + `/verify` evaluators) must satisfy both.
...
2. **LLM-judge half (FR-43).** Dispatch a reviewer subagent with: the path of the skill's `SKILL.md`; the path of each `reference/` file (the reviewer reads them itself — subagents share the filesystem, and step 3's quote validation greps the orchestrator's *own* read of each file, so pasting file bodies into the prompt buys nothing); and the `[J]` check list from `reference/skill-eval.md` (each: `check_id`, the rule text, the *why*, the *how-to-verify*, the pass-condition). The reviewer **makes no edits** (D10 — `/execute` is the only writer). It runs at `temperature: 0` and returns a JSON array — one object per check, **exactly** the given `check_id` set: `{check_id, verdict: 'pass'|'fail', fix_note: '<concrete edit; required & non-empty on fail>', quote: '<≥40-char verbatim substring of the file the check is about; required>'}`. A `fail` whose `quote` is empty or not a verbatim substring of the named file is **treated as `pass`** (FR-43).
...
- **Re-run the `[D]` half of `reference/skill-eval.md`** (re-run `feature-sdlc/tools/skill-eval-check.sh` — a final idempotent deterministic gate, independent of Phase 6a's run; the `[J]` judge half ran once in Phase 6a and is NOT re-dispatched — its accepted fails carry forward as residuals) and **reconcile against `state.yaml.phases.skill-eval.skill_eval.accepted_residuals[]`** (FR-50):
...
This phase is the `/feature-sdlc` orchestrator entrypoint that `/comments resolve` (T10) dispatches into when walking open threads in either orchestrator artifact's inline `pmos-comments` JSON block. The contract — input/output JSON shapes, closed `error_enum` set, idempotency rules, subagent invocation convention — lives in the shared contract doc and is the single source of truth:
