...
**Driver-identity contract.** The build driver claims with a **stable per-loop holder id** — `build:<root-session-id>` (the harness session id, which `/loop` preserves across ticks), overridable with `--holder <id>`. Stability across ticks is what lets tick N+1 recognize tick N's abandoned claim as *mine* and reclaim it immediately (no 4h TTL wait) via `claim-lock.js`'s own-holder reclaim trigger (D3). Single-driver is the committed model — one `/loop` at a time; the only concurrent executor to respect is a manual `build --story <id>` (a *different* holder), which the ownership guard (D2) skips rather than stomps. Thread this holder through every `/backlog claim` and `/backlog next --claim` in this iteration.
