...
Otherwise (`mode == interactive`, Tier 2+) **run `/grill` as a real interactive interview — its standalone one-question-at-a-time behavior — by default; there is no opt-in gate.** Invoke `/pmos-toolkit:grill` against the artifact under review — `01_requirements.html` in the feature / skill pipelines, `02_design.html` in `route: skill` define (`#define-route-skill`) — and let it interview the user directly. The interview is blocking by design: an interactive run is exactly when the user wants to be grilled, so the orchestrator does **not** substitute the silent reviewer-subagent pass here. (The reviewer-subagent quote-grounding contract in `_shared/reviewer-protocol.md` is unchanged and still governs the reviewer dispatches that remain elsewhere — e.g. `/verify`'s artifact reviewers and the Phase 6a `/skill-eval` judge; it simply no longer runs at this phase, because interactive mode now gets the live interview instead.)

After completion: capture `<feature_folder>/grills/<YYYY-MM-DD>_<artifact-stem>.{html,md}` to state (`<artifact-stem>` is `01_requirements` or, in `route: skill` define, `02_design`). On failure: soft-phase failure dialog (Skip SHOWN).
...
5. **Definition merge (docs-only, D20) → exit + teardown.** Before merging `define/<epic-id>` to main, run two deterministic gates:
   ...
   No version bump, no tag. On both gates passing, merge. **The backlog index is derived on read, so there is nothing to regenerate** — bare `/backlog` recomputes the view from `backlog/items/*.md` per `_shared/tracker-crudl.md` §5 (INV-1; no committed `INDEX.md`, D3/L3). Still **do not trust the merged text**: assert uniqueness post-merge with `node plugins/pmos-toolkit/skills/feature-sdlc/scripts/check-id-uniqueness.mjs post-merge <root>/backlog/items` (it reads `items/`, exit 3 ⇒ abort the define run, the merge introduced a duplicate). Epic → `defined`.
