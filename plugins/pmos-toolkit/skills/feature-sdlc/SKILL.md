---
name: feature-sdlc
description: End-to-end SDLC orchestrator. `/feature-sdlc <idea>` turns an idea (text or doc) into a shipped feature by driving the pmos-toolkit pipeline — worktree, optional /ideate for fuzzy seeds, requirements, grill, optional creativity/wireframes/prototype, spec, plan, execute, verify, complete-dev — auto-tiering each stage and persisting resumable state. `/feature-sdlc skill <description>` and `/feature-sdlc skill --from-feedback <text|path|--from-reflect>` author or revise skills via the same pipeline, scored against a binary skill-eval rubric before merge. `/feature-sdlc prototype <seed>` runs the discovery half only (requirements → grill → spec → wireframes → prototype) and stops. `/feature-sdlc list` shows in-flight worktrees. `/skill-sdlc` and `/prototype-sdlc` are thin aliases. Triggers — "build this feature end-to-end", "run the full SDLC", "take this idea through to ship", "/feature-sdlc", "drive the pipeline for me", "create a skill", "author a new skill", "build me a slash command", "turn this workflow into a skill", "apply this retro feedback to the skill", "I have a half-formed idea", "prototype this idea end-to-end", "stakeholder-walkable prototype".
user-invocable: true
