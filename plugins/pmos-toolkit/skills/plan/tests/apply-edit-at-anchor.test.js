#!/usr/bin/env node
// T18 — per-skill contract test for /plan's "Apply comment-resolver edit" shim.
...
const FIXTURE = path.join(__dirname, "fixtures", "apply-edit-at-anchor", "plan_mini.html");
  ...
  return Object.assign(
    {
      ...
      thread_id: "T_A",
      anchor: { id_anchor: "overview", quote_anchor: null },
...
(async () => {
  ...
  if (failures.length > 0) {
    console.error("FAIL: /plan apply-edit-at-anchor — " + failures.length + " of 5 cases");
    for (const f of failures) console.error("  - " + f);
    process.exit(1);
  }
  console.log("PASS: /plan apply-edit-at-anchor — 5 cases");
})().catch((e) => {
