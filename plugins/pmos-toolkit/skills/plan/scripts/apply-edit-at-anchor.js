#!/usr/bin/env node
// T18 — /plan "Apply comment-resolver edit" Node-callable shim.
//
...
// /plan's per-skill apply-edit phase. Surface area kept ≤ ~200 LOC.
...
// Heuristic: out-of-scope restructure of phased task lists.
// /plan artifacts are structural (phases + tasks); rewriting multiple
// numbered sections is infeasible for the shim.
...
async function apply(input) {
  try {
    ...
    const annotation = `<!-- comment-resolver: thread=${input.thread_id} skill=plan request="${bodyExcerpt}" -->\n`;
