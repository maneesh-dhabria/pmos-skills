...
- **Top-level keys:** `story:` = the story id (the `--backlog <id>` when present, else the feature slug); `spec:` = the relative path to the epic spec — `../../02_spec.html` under the per-epic `stories/{story}/` layout, or the local `02_spec.html` when the feature folder is flat. **For a `route: skill` story** the epic carries a `design_doc:` instead of a `spec_doc:` (G2), so `spec:` points at `../../02_design.html` — the design doc *is* the spec this story implements.
...
- **`route: skill` (G2):** the `<path-to-spec-doc>` argument is the epic's `02_design.html` (the `design_doc:`), not a `/spec` output — `/plan` studies it exactly as it would a spec (the same code-study, decision-log, and TDD-task discipline apply). The story's acceptance criteria are the approved findings for that skill plus `skill-patterns.md`; cite `design_doc:` anchors in `**Spec refs:**`. No other behavioural change.
