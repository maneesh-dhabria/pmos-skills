#!/usr/bin/env node
// T18 — /artifact "Apply comment-resolver edit" Node-callable shim.
//
...
// /artifact's per-skill apply-edit phase. Surface area kept ≤ ~200 LOC.
...
// Heuristic: out-of-scope restructure spanning multiple artifact sections.
// /artifact produces arbitrary HTML (PRD, EDD, etc.); rewriting multiple
// cross-cutting sections is infeasible for the shim.
...
async function apply(input) {
  try {
    ...
    //    anchor element capturing the operator's request.
    _applied.add(key);
    const r = resolved.dom_range;
    const bodyExcerpt = String(input.body || "").slice(0, 80).replace(/-->/g, "--&gt;");
    const annotation = `<!-- comment-resolver: thread=${input.thread_id} skill=artifact request="${bodyExcerpt}" -->\n`;
