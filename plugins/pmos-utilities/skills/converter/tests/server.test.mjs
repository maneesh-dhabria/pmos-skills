...
async function main() {
  ...
  check('GET /conversions returns the registry ids (4 data pairs + the PDF pair)',
    conv.status === 200 && JSON.stringify(ids) === JSON.stringify(['csv→json', 'json→csv', 'json→yaml', 'md→pdf', 'pdf→md', 'yaml→json']));
